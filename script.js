...
const telegramRedirectUrl = waitlistDialog?.dataset.telegramUrl || 'https://telegram.me/DanielaMarketing_bot';
...
waitlistForm?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const token = btoa(entry.email.split('').reverse().join(''))
      .replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
    
    const botUrl = `${telegramRedirectUrl}?start=w57259357--${token}`;
